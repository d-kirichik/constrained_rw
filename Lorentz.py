...
class Lorentz:
    ...
    def generate_normalized(self, dt, steps):
      data = self.generate(dt, steps)
      minmax_scale(data, feature_range=(-1,1), axis=0, copy=False)
      minmax_scale(data, feature_range=(-1,1), axis=1, copy=False)
      minmax_scale(data, feature_range=(-1,1), axis=2, copy=False)
      return data
